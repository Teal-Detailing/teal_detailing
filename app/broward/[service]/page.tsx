...
export function generateMetadata({ params }: { params: { service: string } }): Metadata {
  ...
  if (!service) return {}
  const title = `${service.name} in Broward County`
  const description = `Professional mobile ${service.name.toLowerCase()} in Broward County — Teal Detailing comes to your home or office in Fort Lauderdale, Hollywood, Pembroke Pines, Weston, and throughout Broward.`
  return {
    title,
    description,
...
export default function BrowardServicePage({ params }: { params: { service: string } }) {
  const service = servicesData[params.service as keyof typeof servicesData]
  if (!service) notFound()
