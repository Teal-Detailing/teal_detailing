...
async function fetchConversationHistory(customerId: string): Promise<string> {
  const webAppUrl = process.env.GOOGLE_SHEETS_WEBAPP_URL;
  if (!webAppUrl) return "";
  try {
...
async function extractLeadInfo(conversationText: string): Promise<Record<string, unknown> | null> {
...
async function upsertLead(customerId: string, username: string, fields: Record<string, unknown>) {
  const webAppUrl = process.env.GOOGLE_SHEETS_WEBAPP_URL;
  if (!webAppUrl) return;
  await fetch(webAppUrl, {
    method: "POST",
  ...
  if (payload.object === "instagram") {
    for (const entry of payload.entry ?? []) {
      for (const event of entry.messaging ?? []) {
        ...
        if (!isEcho) {
          const conversationText = await fetchConversationHistory(customerId);
          const leadInfo = await extractLeadInfo(conversationText);
          if (leadInfo) {
            await upsertLead(customerId, username ?? customerId, leadInfo);
          }
        }
      }
    }
