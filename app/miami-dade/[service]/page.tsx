...
export function generateMetadata({ params }: { params: { service: string } }): Metadata {
  ...
  if (!service) return {}
  const title = `${service.name} in Miami-Dade County`
  const description = `Professional mobile ${service.name.toLowerCase()} in Miami-Dade County — Teal Detailing comes to your home or office in Brickell, Coral Gables, Doral, Kendall, and throughout Miami-Dade.`
  return {
    title,
    description,
...
export default function MiamiDadeServicePage({ params }: { params: { service: string } }) {
  const service = servicesData[params.service as keyof typeof servicesData]
  if (!service) notFound()
